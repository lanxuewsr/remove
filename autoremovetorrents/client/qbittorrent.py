...
class qBittorrent(object):
    ...
    def remove_torrents(self, torrent_hash_list, remove_data):
        for torrent_hash in torrent_hash_list:
            self._request('command/deletePerm' if remove_data else 'command/delete',
                data={'hashes': torrent_hash})
        return (torrent_hash_list, [])

    def pause_torrents(self, torrent_hash_list):
        """
        Pause torrents and wait for tracker update
        """
        for torrent_hash in torrent_hash_list:
            # 1. 发送暂停请求
            self._request('command/pause', data={'hashes': torrent_hash})
            # 2. 强制更新tracker
            self._request('command/reannounce', data={'hashes': torrent_hash})
            # 3. 等待tracker响应（检查tracker状态）
            max_wait = 30  # 最多等待30秒
            while max_wait > 0:
                trackers = self._request('query/propertiesTrackers/'+torrent_hash).json()
                if any(t.get('status', 0) == 2 for t in trackers):  # 2表示已更新
                    break
                time.sleep(1)
                max_wait -= 1
