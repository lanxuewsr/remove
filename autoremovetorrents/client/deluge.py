...
class Deluge(object):
    ...
    def pause_torrents(self, torrent_hash_list):
        ...
        for torrent_hash in torrent_hash_list:
            # 1. 发送暂停请求
            self._call('core.pause_torrent', [torrent_hash])
            # 2. 强制更新tracker
            self._call('core.force_reannounce', [torrent_hash])
            # 3. 等待tracker响应
            max_wait = 30  # 最多等待30秒
            while max_wait > 0:
                status = self._call('core.get_torrent_status', torrent_hash, ['tracker_status'])
                if 'Announce OK' in status.get('tracker_status', ''):
                    break
                time.sleep(1)
                max_wait -= 1
